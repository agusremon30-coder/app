...
import { ArrowLeft, Terminal, Bug, Shield, Lock, Code, Zap } from 'lucide-react';
    ...
    <div className="min-h-screen pt-20 pb-16">
      <div className="bg-[#0a0a0a] border-b border-[#333]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          ...
          <h1 className="font-orbitron text-3xl sm:text-4xl text-white mb-2">
            Command <span className="text-[#ff0000]">Injection</span>
          </h1>
          <p className="text-[#999] max-w-2xl">
            Execute arbitrary system commands through vulnerable applications.
            Learn filter evasion methods and advanced injection techniques.
          </p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[
            { name: 'Basic Injection', icon: Terminal, desc: 'Simple command injection' },
            { name: 'Blind Injection', icon: Bug, desc: 'Time-based blind injection' },
            { name: 'Space Bypass', icon: Code, desc: 'Bypass space filters' },
            { name: 'Blacklist Bypass', icon: Shield, desc: 'Evade keyword filters' },
            { name: 'Encoded Commands', icon: Lock, desc: 'Use encoding to bypass' },
            { name: 'Chained Commands', icon: Zap, desc: 'Chain multiple commands' },
          ].map((item) => (
