...
import { ArrowLeft, Key, Bug, Shield, Lock, Eye, Code } from 'lucide-react';
...
const JwtLab = () => {
  const simulateAttack = (type: string) => {
    ...
    setTimeout(() => {
      toast.success(`JWT vulnerability exploited!`);
    }, 1500);
  };
    ...
    <div className="min-h-screen pt-20 pb-16">
      <div className="bg-[#0a0a0a] border-b border-[#333]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          ...
          <h1 className="font-orbitron text-3xl sm:text-4xl text-white mb-2">
            JWT Security <span className="text-[#ff0000]">Lab</span>
          </h1>
          <p className="text-[#999] max-w-2xl">
            Exploit JSON Web Tokens through signature bypass, algorithm confusion, and key injection.
            Learn to secure JWT implementations effectively.
          </p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[
            { name: 'None Algorithm', icon: Code, desc: 'Exploit alg: none vulnerability' },
            { name: 'Algorithm Confusion', icon: Shield, desc: 'RS256 to HS256 confusion' },
            { name: 'Weak Secret', icon: Lock, desc: 'Crack weak JWT secrets' },
            { name: 'Key Injection', icon: Key, desc: 'Inject malicious JWK keys' },
            { name: 'Token Manipulation', icon: Eye, desc: 'Modify token payload' },
            { name: 'Kid Injection', icon: Bug, desc: 'Exploit kid header parameter' },
          ].map((item) => (
