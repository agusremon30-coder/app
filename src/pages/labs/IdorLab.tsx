...
import { ArrowLeft, Eye, Bug, Shield, User, FileText, CreditCard } from 'lucide-react';
...
const IdorLab = () => {
  const simulateAttack = (type: string) => {
    ...
    setTimeout(() => {
      toast.success(`Vulnerability found in ${type}!`);
    }, 1500);
  };
    ...
    <div className="min-h-screen pt-20 pb-16">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[
            { name: 'User ID Manipulation', icon: User, desc: 'Access other user profiles by changing IDs' },
            { name: 'Document Access', icon: FileText, desc: 'View documents belonging to other users' },
            { name: 'Order Information', icon: CreditCard, desc: 'Access order details of other customers' },
            { name: 'API Endpoint Testing', icon: Eye, desc: 'Test API endpoints for authorization flaws' },
            { name: 'Mass Assignment', icon: Shield, desc: 'Modify object properties without authorization' },
            { name: 'UUID Prediction', icon: Bug, desc: 'Predict and access resources via UUIDs' },
          ].map((item) => (
