...
import { ArrowLeft, Shield, Bug, Lock, Code, Globe, Zap } from 'lucide-react';
...
const CspBypassLab = () => {
  const simulateAttack = (type: string) => {
    ...
    setTimeout(() => {
      toast.success(`CSP bypass successful! Script executed.`);
    }, 1500);
  };
    ...
    <div className="min-h-screen pt-20 pb-16">
      <div className="bg-[#0a0a0a] border-b border-[#333]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          ...
          <h1 className="font-orbitron text-3xl sm:text-4xl text-white mb-2">
            CSP <span className="text-[#ff0000]">Bypass</span>
          </h1>
          <p className="text-[#999] max-w-2xl">
            Bypass Content Security Policy restrictions to execute injected scripts.
            Learn various CSP bypass techniques and misconfigurations.
          </p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[
            { name: 'Unsafe Inline', icon: Code, desc: 'Exploit unsafe-inline directive' },
            { name: 'Unsafe Eval', icon: Zap, desc: 'Use unsafe-eval for code execution' },
            { name: 'Wildcard Domain', icon: Globe, desc: 'Abuse wildcard in script-src' },
            { name: 'JSONP Endpoint', icon: Bug, desc: 'Use JSONP endpoints for bypass' },
            { name: 'Angular CSP', icon: Shield, desc: 'Bypass CSP with Angular' },
            { name: 'Base Tag', icon: Lock, desc: 'Exploit base tag injection' },
          ].map((item) => (
