...
import { ArrowLeft, Shield, Bug, Lock, Cookie, Key, Globe } from 'lucide-react';
...
const CsrfLab = () => {
  const simulateAttack = (type: string) => {
    ...
    setTimeout(() => {
      toast.success(`${type} vulnerability exploited!`);
    }, 1500);
  };
    ...
    <div className="min-h-screen pt-20 pb-16">
      <div className="bg-[#0a0a0a] border-b border-[#333]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          ...
          <h1 className="font-orbitron text-3xl sm:text-4xl text-white mb-2">
            CSRF <span className="text-[#ff0000]">Protection Bypass</span>
          </h1>
          <p className="text-[#999] max-w-2xl">
            Cross-Site Request Forgery attacks and protection bypass techniques.
            Learn to bypass CSRF tokens, SameSite cookies, and other protections.
          </p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[
            { name: 'Token Bypass', icon: Key, desc: 'Bypass CSRF token validation' },
            { name: 'SameSite Bypass', icon: Cookie, desc: 'Exploit SameSite cookie policies' },
            { name: 'Referer Bypass', icon: Globe, desc: 'Bypass Referer header checks' },
            { name: 'Double Submit', icon: Lock, desc: 'Exploit double submit cookie pattern' },
            { name: 'Custom Header', icon: Shield, desc: 'Bypass custom header verification' },
            { name: 'JSON CSRF', icon: Bug, desc: 'CSRF against JSON endpoints' },
          ].map((item) => (
