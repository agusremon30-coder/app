...
import { ArrowLeft, Folder, Bug, Shield, Lock, FileCode, FolderOpen } from 'lucide-react';
import { Link } from 'react-router-dom';
import { toast } from 'sonner';

const PathTraversalLab = () => {
    ...
    <div className="min-h-screen pt-20 pb-16">
      <div className="bg-[#0a0a0a] border-b border-[#333]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          ...
          <h1 className="font-orbitron text-3xl sm:text-4xl text-white mb-2">
            Path <span className="text-[#ff0000]">Traversal</span>
          </h1>
          <p className="text-[#999] max-w-2xl">
            Access files outside the web root directory using directory traversal techniques.
            Learn various bypass methods for path validation.
          </p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[
            { name: 'Basic Traversal', icon: Folder, desc: 'Simple ../ sequences' },
            { name: 'Encoding', icon: FileCode, desc: 'URL and Unicode encoding' },
            { name: 'Null Byte', icon: Bug, desc: 'Null byte truncation' },
            { name: 'Double Encoding', icon: Shield, desc: 'Double URL encoding' },
            { name: 'Absolute Path', icon: FolderOpen, desc: 'Use absolute paths' },
            { name: 'Filter Bypass', icon: Lock, desc: 'Bypass path filters' },
          ].map((item) => (
...
export default PathTraversalLab;
